...
local1 <- local1[,short_names]

local2 <- read.csv(paste0(path_name,directory_names[2],"/metrics-local-orig.csv"), skip=1) #RUNTIME ~ 00:28 mins
local2 <-local2[,c(intersect(names(local2),names(local1)))]
local2 <- local2[,short_names]

local3 <- read.csv(paste0(path_name,directory_names[3],"/metrics-local-orig.csv"), skip=1) #RUNTIME ~ 00:28 mins
local3 <-local3[,c(intersect(names(local3),names(local1)))]
local3 <- local3[,short_names]

proj4 <- read.csv(paste0(path_name,directory_names[1],"/metrics-local.csv"), nrows=1, header = FALSE)
