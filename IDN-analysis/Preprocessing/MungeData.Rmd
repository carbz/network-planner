...
Load Libraries 
```{r Load Libraries, message = F, cache=TRUE}
...
```{r ImportData, cache=TRUE, fig.width=8}
...
ge <- google_earth_plot(all_centroids)
...
```{r ImportBIGdata, cache=TRUE, fig.width=8}
#original processed BIG data had some gaps but was usable after some massaging 
#included (1)Unique Cluster Code & (2) Some designated geographic areas, big file w/ ~17,000 observations!
# cluster_points <- read.csv("~/Dropbox/Indonesia Geospatial Analysis/Data Modeling and Analysis/NPinputs/Jan2014-Preprocessing/cluster_points_NTT+MMU-updated20140109+Clust_Code.csv", header = T)

#Updated January 14.  Shaky identified projection issue with original lat/long points which is now resolved
cluster_points <- read.csv("~/Dropbox/Indonesia Geospatial Analysis/Data Modeling and Analysis/NPinputs/Jan2014-Preprocessing/ShakyUpdata-20140114/all_big_settlement_points_with_pop_01142014_corrected+Clust_Code.csv", header = T)
...
##use unique Clust_Code field for cluster_points to give each obs a unique ID with IDSP no. as prefix
...
ggplot(cluster_points, aes(x = Longitude, y = Latitude)) + 
  geom_point(colour = 'red') + 
  labs(title = "BIG Settlement Location") +
  coord_equal()
...
```{r Import_BPS_Census_data, cache=TRUE, fig.width=8}
...
```{r CompositeView_IDNSettlements, cahce=TRUE, fig.width=10}
...
cluster_column_namesV2 <- colnames(cluster_points)[which(colnames(cluster_points) %in% colnames(all_centroids))]


#Develop dataframe that will fill Desa cluster gaps from the census file
desa_gap_fillers <- subset(all_centroids, IDSP2010 %in% desa_gaps, c(cluster_column_namesV2, "population", "NUM_HHOLDS"))
...
#this is not needed, info carried through now... 
...
composite_settlements[which(is.na(composite_settlements$TOT_HHOLD)),'TOT_HHOLD'] <- 
    composite_settlements[which(is.na(composite_settlements$TOT_HHOLD)),'NUM_HHOLDS'] 
composite_settlements<- mutate(composite_settlements, 'target_household_penetration_rate'= (1-PLN_HHOLD))
...
ggplot(composite_settlements, aes(x = Longitude, y = Latitude))+ geom_point(aes(colour = XY_Source)) + labs(title = "Composite View of Settlement Locations")
table(composite_settlements$XY_Source)

#Visual check on geogpraphic divisions 
ggplot(composite_settlements, aes(x = Longitude, y = Latitude))+ geom_point(aes(colour = EI_SubArea)) + labs(title = "Geographic Divsions of Settlement Locations")
#Visual check on PLN Area Territories 
summary(composite_settlements$EI_SubArea)

ggplot(composite_settlements, aes(x = Longitude, y = Latitude)) + 
  geom_point(aes(colour = PLN_Cabang)) + 
  labs(title = "PLN Area Offices") + 
  coord_equal()
...
df <- subset(composite_settlements, select = c("Name", "Latitude", "Longitude", "pop", "ho_size", "EI_SubArea", "PLN_Cabang", "PROVINSI", "XY_Source", "target_household_penetration_rate", "full_population"))
composite_settlements <- df
# 
# #Output entire field
# write.csv(composite_settlements, "~/Dropbox/Indonesia Geospatial Analysis/Data Modeling and Analysis/NPinputs/Jan2014-Preprocessing/CompositeDemographicFile.csv", row.names=F)
# 
#Output Geographic Area Division
...
for (i in 1:length(Regions)){
  ...
  write.csv(subset_area, str_c(Regions[i],"-demographic_data_with_centroids_for_gaps-20140206.csv"), row.names = F)
}
# flores <- c("FloresTimu", "FloresBara")
# flores_subset <- subset(composite_settlements, EI_SubArea %in% flores)
# write.csv(flores_subset, "Flores-demographic_data_with_centroids_for_gaps-20140206.csv", row.names = F)
...
for (i in 1:length(cabangs)){
  ...
  write.csv(subset_area, str_c(cabangs[i],"-demographic_data_with_centroids_for_gaps-20140206.csv"), row.names = F)
}
